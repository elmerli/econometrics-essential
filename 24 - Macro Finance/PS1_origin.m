...
eta_s = kappa * (1-ell);
q_s   = (2*kappa-kappa.^2)./(eta_s * (rho_e-rho_h)+rho_h); % GM condition on slides pg 27
  % cmt: global solution can't use drift condition, only holds in SS
...
q_span_case1 = (2*kappa_span_case1-kappa_span_case1.^2)./(eta_span * (rho_e-rho_h)+rho_h); % GM condition on slides pg 27
mu_eta_geo_case1 = (1-eta_span).*(-(rho_e-rho_h) + ((1-kappa_span_case1)./q_span_case1).* kappa_span_case1./eta_span);


%% Solve for q less binding
%-------------------------------------------%
...
for i = 1:99
    eta_t = eta_span(i);
    F_obj = @(x) [(2 * x(1) - x(1).^2)./(eta_t * (rho_e-rho_h)+rho_h) - x(2);... % GM condition on slides pg 27
                  1 - x(1)*(1-ell * q_s/x(2))./eta_t]; % q* constraint on slides pg 33
    x0    = [eta_t/(1-ell);20];
    x_sol = fsolve(F_obj,x0,options);
    kappa_span_case2(i) = x_sol(1);
end

kappa_span_case2(kappa_span_case2>=1) = 1; % short selling constraint on capital
