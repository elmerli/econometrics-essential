...
quantmod::getSymbols(symbol.vec, from ="2000-01-03", to = "2012-04-03") #  load data from various sources
...
MSFT.ret = PerformanceAnalytics::CalculateReturns(MSFT, method="log")
GSPC.ret = PerformanceAnalytics::CalculateReturns(GSPC, method="log")
...
colnames(dataToPlot) = c("Returns", "Returns^2", "abs(Returns)")
zoo::plot.zoo(dataToPlot, main="GSPC Daily Returns", col="blue")

# plot autocorrelations of returns, returns^2 and abs(returns)
par(mfrow=c(3,2)) #  set or query graphical parameters
  stats::acf(MSFT.ret, main="MSFT Returns") # acf computes (and by default plots) estimates of the autocovariance or autocorrelation function.
...
PerformanceAnalytics::table.Stats(MSFT.GSPC.ret)
...
?ugarchspec # Univariate GARCH Specification
