...
## Comment: 
	...
	# but then we can see that the AIC is 30 now,lower than before which is 32. So the model is performing better. 
...
pr.knn_class <- as.integer(as.character(pr.knn))
...
## Comment: From the plotted graph we can see that the MSE is minimized at k=5, so we choose k=5 in this case

pr.knn <- knn(knntrain,knntest,knntrain$crime_compare, k = 5)
table(pr.knn,knntest$crime_compare)
pr.knn_class <- as.integer(as.character(pr.knn))
...
## Comment: we can see that the MSE for kNN with k set at 5 is 0.17, which less than both LDA and QDA
...
smoke_1 <- smoke %>%
	...
	dplyr::mutate(smokes = ifelse(cigs>0,1,0), motheduc = as.integer(as.character(motheduc)))
...
p_edu12 <- predict(smoke_preg.probit,smoke_12)
...
## Comment: 
	...
	# and smoking, such as health habits(eating habits etc.) which will influcence both one's income and possibility to smoke
