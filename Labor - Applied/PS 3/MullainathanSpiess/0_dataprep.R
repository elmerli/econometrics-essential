...
setwd("/Users/zongyangli/Google Drive/Cornell PhD/3rd Semester/Labor Seminar/PS/PS 3/AHS 2011 National and Metropolitan PUF v1.4 CSV")
...
# Read in data
newhouse <- read.csv(file="tnewhouse.csv", quote="\"'") # 186448 observations, 909 variables
...
newhouse <- newhouse[newhouse$VALUE > 0,] # 91712 dropped
...
ahs <- newhouse[ newhouse$TENURE=="1" &
                 newhouse$VACANCY == -6 &
                   newhouse$NATLFLAG == 2,
                  keepvars] # keep only the chosen vars
...
# Take samples
set.seed(20170313)
ahs$holdout <- as.logical(1:nrow(ahs) %in% sample(nrow(ahs), nrow(ahs) - 10000)) # hold out sample for testing

# Create folds for lasso barcode plot
ahs$lassofolds <- as.factor(ceiling(10 * sample(nrow(ahs)) / nrow(ahs))) # random generate # 1-10, assign to each row
...
saveRDS(list(df=ahs,vars=vars,getformula=getformula),
        file="/Users/zongyangli/Google Drive/Cornell PhD/3rd Semester/Labor Seminar/PS/PS 3/ahs2011forjep.rdata")
